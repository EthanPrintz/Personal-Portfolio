(function(win, doc) {
  ...
  function handleHover(target, container, config, touch) {
    ...
    if (config.transition && typeof config.transition === "object") {
      ...
      target.style[transitionDurationProp] = getUnit(
    ...
    if (config.shadow) {
      ...
      if (config.transition && typeof config.transition === "object") {
        ...
        _shadow.style[transitionTimingProp] = getTFunc(
    ...
    if (config.shine) {
      ...
      if (config.transition && typeof config.transition === "object") {
        ...
        _shine.style[transitionDurationProp] = getUnit(
          config.transition.duration
        );
    ...
    function leave() {
      ...
      if (config.hoverClass && config.hoverOutClass) {
        ...
        setTimeout(function() {
          target.className = removeClass(
            target.className,
            config.hoverOutClass
          );
        }, 1000);
      } else if (config.hoverClass) {
        target.className = removeClass(target.className, config.hoverClass);
